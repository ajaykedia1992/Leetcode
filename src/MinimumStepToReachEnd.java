...
public class MinimumStepToReachEnd {
	...
	public static void main(String args[]) {
		...
		System.out.println("Minimum Initial Points Required: " + minInitialPoints1(points, R, C));
	}

	private static int minInitialPoints1(int[][] points, int r, int c) {
		int[][] dp = new int[r][c];
		int m = r, n = c;

		dp[m - 1][n - 1] = points[m - 1][n - 1] > 0 ? 1 : Math.abs(points[m - 1][n - 1]) + 1;

		// filling the right most column
		...
		for (int i = n - 2; i >= 0; i--) {
			dp[m - 1][i] = Math.max(dp[n - 1][i + 1] - points[m - 1][i], 1);
		}

		for (int i = m - 2; i >= 0; i--) {
			for (int j = n - 2; j >= 0; j--) {
				int minPoint = Math.min(dp[i + 1][j], dp[i][j + 1]);
				dp[i][j] = Math.max(minPoint - points[i][j], 1);
			}
		}
